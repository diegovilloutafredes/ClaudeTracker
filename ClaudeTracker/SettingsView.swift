...
struct SettingsView: View {
    ...
    var body: some View {
        ...
        Form {
            ...
            if viewModel.isAuthenticated {
                if viewModel.accounts.filter({ $0.claudeCodeLinked }).count >= 2 {
                    balanceSection
                }
                displaySection
                resetSection
    ...
    /// Strategy picker tag — used as the SwiftUI Picker selection. Maps onto
    /// `BalanceStrategy` cases without exposing the associated values, which
    /// are surfaced in the conditional sub-controls below.
    private enum StrategyTag: String, CaseIterable, Hashable {
        case manual, prioritize, balance

        var label: LocalizedStringKey {
            switch self {
            case .manual: return "Manual"
            case .prioritize: return "Prioritize one account"
            case .balance: return "Balance across accounts"
            }
        }
    }

    /// Selection helper that reads/writes `viewModel.balanceSettings.strategy`
    /// via the tag enum so the Picker doesn't have to deal with associated values.
    private var strategyTagBinding: Binding<StrategyTag> {
        Binding(
            get: {
                switch viewModel.balanceSettings.strategy {
                case .manual: return .manual
            ...
            set: { newTag in
                let linked = viewModel.accounts.filter { $0.claudeCodeLinked }
                switch newTag {
                case .manual:
                    viewModel.balanceSettings.strategy = .manual
                case .prioritize:
                    // Default preferred = active account if linked, otherwise first linked.
                    let preferred = linked.first(where: { $0.id == viewModel.activeAccountID })
                        ?? linked.first
                    if let p = preferred {
                        viewModel.balanceSettings.strategy = .prioritize(preferredID: p.id, threshold: 80)
                    }
                case .balance:
                    viewModel.balanceSettings.strategy = .balance(hysteresis: 8)
                }
                BalanceSettingsStore.save(viewModel.balanceSettings)
            }
        )
    }

    private var balanceSection: some View {
        Section("Auto-balance Claude Code") {
            Picker("Strategy", selection: strategyTagBinding) {
                ForEach(StrategyTag.allCases, id: \.self) { tag in
            ...
            Toggle("When you run `claude` (recommended)", isOn: hookToggleBinding)
                .toggleStyle(GreenSwitchStyle())
            Text("Requires installing the `claude-balance` shell hook — see Help.")
                .font(.caption)
                .foregroundStyle(.secondary)
        }
    }

    /// Conditional sub-controls per strategy. Reads/writes the associated values
    /// of the current strategy case.
    @ViewBuilder
    private var balanceSubControls: some View {
        switch viewModel.balanceSettings.strategy {
        case .manual:
            EmptyView()
        case .prioritize(let preferredID, let threshold):
            Picker("Preferred account", selection: prioritizePreferredBinding(currentID: preferredID, threshold: threshold)) {
                ForEach(viewModel.accounts.filter({ $0.claudeCodeLinked })) { acct in
                    Text(acct.label).tag(acct.id)
                }
            }
            HStack {
                Text("Switch when above")
                Slider(value: prioritizeThresholdBinding(preferredID: preferredID, currentThreshold: threshold),
                       in: 50...95, step: 5)
                Text("\(Int(threshold))%")
        ...
        case .balance(let hysteresis):
            Stepper(value: balanceHysteresisBinding(currentHysteresis: hysteresis), in: 1...25, step: 1) {
                Text("Switch threshold: \(Int(hysteresis)) pp")
            }
        }
    }

    private func prioritizePreferredBinding(currentID: UUID, threshold: Double) -> Binding<UUID> {
        Binding(
            get: { currentID },
            set: { newID in
                viewModel.balanceSettings.strategy = .prioritize(preferredID: newID, threshold: threshold)
                BalanceSettingsStore.save(viewModel.balanceSettings)
            }
        )
    }

    private func prioritizeThresholdBinding(preferredID: UUID, currentThreshold: Double) -> Binding<Double> {
        Binding(
            get: { currentThreshold },
            set: { newThreshold in
                viewModel.balanceSettings.strategy = .prioritize(preferredID: preferredID, threshold: newThreshold)
                BalanceSettingsStore.save(viewModel.balanceSettings)
            }
        )
    }

    private func balanceHysteresisBinding(currentHysteresis: Double) -> Binding<Double> {
        Binding(
            get: { currentHysteresis },
            set: { newH in
                viewModel.balanceSettings.strategy = .balance(hysteresis: newH)
                BalanceSettingsStore.save(viewModel.balanceSettings)
            }
        )
    }
    ...
    private var hookToggleBinding: Binding<Bool> {
        Binding(
            get: { viewModel.balanceSettings.triggerOnClaude },
            set: { v in
                viewModel.balanceSettings.triggerOnClaude = v
                BalanceSettingsStore.save(viewModel.balanceSettings)
            }
        )
    }
